...
class InteractiveMode:
    ...
    def _search_and_download(self):
        """Search, play or download anime workflow."""
        from .commands import choose_anime
        selected_anime_list = choose_anime(self.api, "", multi=True)
    ...
    def _play_selected_anime(self):
        """Play selected anime episodes via streaming."""
        console.print(f"\n[bold]Selected {len(self.selected_anime)} anime[/bold]")

        self._configure_download_options()
        quality_value = self.download_options["quality"]
        audio_value = self.download_options["audio"]
        episodes = self.download_options["episodes"]

        from .commands import detect_media_player, play_episode_stream

        media_player = detect_media_player(self.config.get("player", ""))
        if not media_player:
            console.print(
                "[red]No compatible media player found. Please install mpv, vlc, ffplay, or mplayer.[/red]"
            )
            return
        ...
        for anime in self.selected_anime:
            ...
            if not episode_data:
                console.print(f"[red]Could not fetch episodes for {anime['title']}.[/red]")
                continue

            filtered_episodes = self._filter_episodes(episode_data, episodes)
            if not filtered_episodes:
                console.print(f"[yellow]No episodes selected for {anime['title']}.[/yellow]")
                continue

            for idx, ep_data in enumerate(filtered_episodes):
                ...
                if not success:
                    console.print(f"[red]Failed to play {anime['title']} Episode {ep_num}[/red]")
                    continue

                if idx < len(filtered_episodes) - 1:
                    next_ep = int(filtered_episodes[idx + 1]["episode"])
                    if not questionary.confirm(f"Play next episode ({next_ep})?").ask():
                        break
